...
const mapper = (elem: CategoryArrayType, index: number) => {
  return <CategoryCard key={index} title={elem.title} imgUrl={elem?.bannerimage?.url ?? "/logo.png"} />;
};
...
function CategoryCard(props: CategoryType) {
  ...
  return (
    <Link href={"/" + title.replaceAll(" ", "-").toLowerCase()} className="img-container category-card">
